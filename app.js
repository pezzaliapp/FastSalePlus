...
<script>
  ...
  function calcolaPrezzo(input) {
    ...
    if (input.classList.contains("prezzoLordo") || input.classList.contains("sconto")) {
      ...
      prezzoNettoCalcolato = Math.round(prezzoNettoCalcolato * 100) / 100;
      // Sovrascriviamo il Prezzo Netto col valore formattato
      prezzoNettoEl.value = formatNumberITA(prezzoNettoCalcolato);
    }

    // Ricalcoliamo (sempre dopo aver letto il nuovo valore netto formattato)
    let prezzoNettoEffettivo = parseNumberITA(prezzoNettoEl.value) || 0;
    
    let prezzoTotale = prezzoNettoEffettivo * quantita;
    // Arrotondiamo a 2 decimali
  ...
  function aggiornaTotaleGenerale() {
    ...
    // Arrotondiamo sempre
    totaleGenerale = Math.round(totaleGenerale * 100) / 100;
  ...
  function calcolaMarginalita() {
    ...
    if (margine > 0) {
      // calcolo col margine, poi arrotondo a 2 decimali
